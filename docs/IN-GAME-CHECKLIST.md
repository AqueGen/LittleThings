...
- [ ] Default position "Slot line, right edge": the icons form one column at the row's right edge (under a transmog addon's corner mark), on every row with or without an armor type. One icon fits right of the armor text, which stays in Blizzard's place. Two or more push the armor text left of them. Nothing is covered. With the module off or on the name line the armor text is back in Blizzard's place.
